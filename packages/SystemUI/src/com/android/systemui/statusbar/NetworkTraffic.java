...
public class NetworkTraffic extends TextView implements TunerService.Tunable,
    ...
    private void displayStatsAndReschedule() {
        ...
        if (mEnabled && mAttached && !mPowerSaveEnabled) {
    ...
    private final BroadcastReceiver mPowerSaveReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            if (PowerManager.ACTION_POWER_SAVE_MODE_CHANGED.equals(intent.getAction())) {
                mTrafficHandler.post(NetworkTraffic.this::updatePowerSave);
            }
        }
    };
    ...
    private void updatePowerSave() {
        ...
        if (mPowerSaveEnabled == isPowerSave) return; 

        mPowerSaveEnabled = isPowerSave;
        if (mPowerSaveEnabled) {
            clearHandlerCallbacks();
            updateVisibility(); // <--- Let the centralized logic hide it
        } else {
            if (mEnabled && mAttached) {
                clearHandlerCallbacks();
                mTrafficHandler.sendEmptyMessage(MESSAGE_TYPE_PERIODIC_REFRESH);
            }
        }
    }
    ...
    private void updateVisibility() {
        // MODIFIED: Added !mPowerSaveEnabled check
        boolean visible = mEnabled && mIsActive && !mPowerSaveEnabled
    ...
    public void onTuningChanged(String key, String newValue) {
        switch (key) {
            ...
            case NETWORK_TRAFFIC_AUTOHIDE_THRESHOLD:
                int autohidethreshold = TunerService.parseInteger(newValue, 0);
